...
export async function createList({ title, icon = '' }) {
  const doc = { _id: `list:${uuid()}`, type: 'list', title, icon, createdAt: now(), updatedAt: now() };
  await db.put(doc); return doc;
}
export async function updateList(id, patch) {
  const cur = await db.get(id); const next = { ...cur, ...patch, updatedAt: now() };
  await db.put(next); return next;
}
...
export async function listLists() {
  ...
  return res.docs.sort((a,b)=> b.updatedAt.localeCompare(a.updatedAt));
}
...
export async function createCard({ listId, categoryId=null, title, description='', tags=[] }) {
  const doc = { _id: `card:${uuid()}`, type: 'card', listId, categoryId, title, description, tags, isFavorite:false, createdAt: now(), updatedAt: now() };
  await db.put(doc); return doc;
}
...
export async function toggleFavoriteCard(id) {
  const cur = await db.get(id); return updateCard(id, { isFavorite: !cur.isFavorite });
}
export async function deleteCard(id) { await db.remove(await db.get(id)); }
